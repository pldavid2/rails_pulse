...
RailsPulse::Schema ||= lambda do |connection|
  ...
  unless connection.table_exists?(:rails_pulse_exception_groups)
    connection.create_table :rails_pulse_exception_groups do |t|
      ...
      t.integer  :occurrence_count, default: 0, comment: "Atomic counter of total occurrences"
      t.string   :status, default: "unresolved", null: false, limit: 20, comment: "unresolved, resolved, or ignored"
      t.string   :source_location, comment: "First app frame file:line for display"
  ...
  unless connection.table_exists?(:rails_pulse_exception_occurrences)
    connection.create_table :rails_pulse_exception_occurrences do |t|
      ...
      t.text     :local_variables, comment: "JSON from TracePoint/VariableSerializer"
      t.text     :custom_context, comment: "JSON hash of custom data"
      t.text     :request_headers, comment: "JSON hash, safe subset only"
      t.text     :user_agent
    ...
    connection.add_index :rails_pulse_exception_occurrences, [:user_type, :user_id], name: "index_rp_exception_occurrences_on_user"
  end
